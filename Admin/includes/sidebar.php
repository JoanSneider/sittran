  <div class=" sidebar" role="navigation">
            <div class="navbar-collapse">
        <nav class="cbp-spmenu cbp-spmenu-vertical cbp-spmenu-left" id="cbp-spmenu-s1">
          <ul class="nav" id="side-menu">
            <li>
              <a href="dashboard.php"><i class="fa fa-home nav_icon"></i>Panel de Control</a>
            </li>
            <li>
              <a href="add-services.php"><i class="fa fa-cogs nav_icon"></i>Servicios<span class="fa arrow"></span> </a>
              <ul class="nav nav-second-level collapse">
                <li>
                  <a href="add-services.php">Agregar Servicio</a>
                </li>
                <li>
                  <a href="manage-services.php">Administrar Servicio</a>
                </li>
              </ul>
              <!-- /nav-second-level -->
            </li>          
            <li>
              <a href="all-appointment.php"><i class="fa fa-check-square-o nav_icon"></i>Agendas<span class="fa arrow"></span></a>
              <ul class="nav nav-second-level collapse">
                <li>
                  <a href="all-appointment.php">Todas las Agendas</a>
                </li>
                <li>
                  <a href="new-appointment.php">Aceptar Agendas</a>
                </li>
                <li>
                  <a href="accepted-appointment.php">Agendas Aceptadas</a>
                </li>
                <li>
                  <a href="rejected-appointment.php">Agendas Rechazadas</a>
                </li>
              </ul>
              <!-- //nav-segundo-nivel -->
            </li>
           
        
            <li>
              <a href="add-customer.php" class="chart-nav"><i class="fa fa-user nav_icon"></i>Agregar Cliente</a>
            </li>
             <li>
              <a href="customer-list.php" class="chart-nav"><i class="fa fa-users nav_icon"></i>Lista de Clientes</a>
            </li>
              <li>
              <a href="#"><i class="fa fa-check-square-o nav_icon"></i>Reportes<span class="fa arrow"></span></a>
              <ul class="nav nav-second-level collapse">
                 <li><a href="bwdates-reports-ds.php"> Facturas por Fecha</a></li>
                   
                    <li><a href="sales-reports.php">Reporte de Ventas</a></li>
              </ul>
              <!-- //nav-second-level -->
            </li>

    <li>
              <a href="invoices.php" class="chart-nav"><i class="fa fa-file-text-o nav_icon"></i>Facturas</a>
            </li>
            <li>
              <a href="search-appointment.php" class="chart-nav"><i class="fa fa-search nav_icon"></i>Buscar Agendas</a>
            </li>
            <li>
              <a href="search-invoices.php" class="chart-nav"><i class="fa fa-search nav_icon"></i>Buscar Facturas</a>
            </li>
          

          </ul>
          <div class="clearfix"> </div>
          <!-- //sidebar-collapse -->
        </nav>
      </div>
    </div>